...
<md-toolbar>
    <div class="md-toolbar-tools">
        <md-button class="md-icon-button" ng-click="toggleSidenav('left')" hide-gt-sm aria-label="Menu" ng-hide="c.toggleSearch">
            <md-icon md-svg-icon="menu"></md-icon>
        </md-button>
        <h2 flex ng-hide="showSearch">Books</h2>
        <md-input-container ng-show="showSearch" flex>
            <label>Search book</label>
            <input type="text" focus-if="showSearch" ng-model="searchQ" ng-focus="searchIsFocused = true;" ng-blur="searchIsFocused = false;">
        </md-input-container>
        <md-button class="md-icon-button" ng-click="toggleSearch()" aria-label="search">
...
<md-content flex layout="row" id="content">
    ...
    <md-list flex ng-hide="isLoadingBooks">
        <div ng-repeat="group in books | orderBy:'bookgroup_id' | groupBy: 'bookgroup_name' | toArray:true track by group.$key">
            ...
            <md-list-item class="md-2-line" ui-sref="book({id:boook.id})" ng-repeat="boook in group | orderBy:'name' | filter:searchQ track by boook.id">
